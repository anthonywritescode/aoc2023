...
def compute(s: str) -> int:
    ...
    supported_by: dict[int, set[int]] = collections.defaultdict(set)

    for i, brick in enumerate(done):
        for j, other in enumerate(done[i + 1:], start=i + 1):
            if other.supported_by(brick):
                supports[i].add(j)
                supported_by[j].add(i)

    total = 0
    for i in range(len(done)):
        if all(len(supported_by[other]) > 1 for other in supports[i]):
            total += 1
